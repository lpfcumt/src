...
<ul class="nav navbar-default nav-pills nav-justified" style="font-size:16px;">
	...
	<li class="active"><a href="Adminstudentsinfo"><span style="color:white;">学生信息</span></a></li>
...
<div class="row">
		...
		<ul class="nav nav-tabs" role="tablist" id="myTab">
                <li role="presentation" class="active"><a href="#all" role="tab" data-toggle="tab" >已激活</a></li>
                <li role="presentation" ><a href="#all1" role="tab" data-toggle="tab" onclick="return unactive()">待激活</a></li>
            </ul>
                    ...
                    <table class="table table-hover table-striped table-bordered" id="table1">
				        <thead>
					       <tr>
						      ...
						      <input type="hidden" value="${sessionScope.pagesize}" id="pagesize"/>
							     学生姓名
						      </th>
                              <th style= "text-align:center;">
							     学号
				        ...
				        <c:forEach items="${sessionScope.listAllStudents}" var="listAllStudents1">
				        <tbody>
					       <tr>
					       <c:if test="${listAllStudents1.state=='已激活'}">
						      <td style= "text-align:center;">
							     ${listAllStudents1.name}
						      </td>
                               <td style= "text-align:center;">
                                 ${listAllStudents1.students_id}
						      </td>
						      <td style= "text-align:center;">
							     <a target="_blank" href="adminstudents?id=${listAllStudents1.id}" >学生信息</a><br>
<!--                                  <a src="#">修改学生信息</a><br> -->
						      </td>
						      <td style= "text-align:center;">
                                 ${listAllStudents1.state}
						      </td>
						     </c:if>
					       </tr>
				        </tbody>
             ...
             <div role="tabpanel" class="tab-pane " id="all1">
                <table class="table table-hover table-striped table-bordered" id="table2">
				        <thead>
					       <tr>
						      <th style= "text-align:center;">
						      <input type="hidden" value="${sessionScope.pagesize1}" id="pagesize1"/>
							     学生姓名
						      </th>
                              <th style= "text-align:center;">
							     学号
	...
	$(".tcdPageCode1").createPage({
        ...
        backFn:function(p){
        	$.ajax({
     		   url:'ListAllStudents1',// 跳转到 action  
     		   data:{  current:p  
     		    }, 
     		    type:'post',  
     		    cache:false,   
     		    ...
     		    success:function(data) { 
     		    	$("#table1 tbody").remove();
     		    	var jsonObj=eval(data.listAllStudents);
     		    	if(jsonObj==null){
     		    		 alert("无学生！");
     		    	}
     		    	else{
     		    		...
     		    		$.each(jsonObj, function (index, item) { 
     		    			 var $tr = $('<tr>');
     		    			$tr.append("<td style= 'text-align:center;'>"+item.name+"</td>");
     		    			$tr.append("<td style= 'text-align:center;'>"+item.students_id+"</td>");
 						    $tr.append("<td style= 'text-align:center;'><a target='_blank' href='adminstudents?id="+item.id+"' >学生信息</a><br></td>");
 						    $tr.append("<td style= 'text-align:center;'><a href='changestate?id="+item.id+"'>"+item.state+"</a></td>");
     		    			 	$tab1.append($tr);
     			    	})
     			    		}; 
     			     }
	...
	function unactive(){
		...
		$.ajax({
			   url:'ListUnActiveStudents',// 跳转到 action  
			   data:{   
			    }, 
			    type:'post',  
			    cache:false,   
			    ...
			    success:function(data) { 
			    	$("#table2 tbody").remove();
			    	var jsonObj=eval(data.listunactivestudents);
			    	if(jsonObj[0]==null){
	...
	$(".tcdPageCode2").createPage({
        pageCount:$("#pagesize1").val(),
        current:1,
        backFn:function(p){
        	$.ajax({
     		   url:'ListUnActiveStudents1',// 跳转到 action  
     		   data:{  current:p  
     		    }, 
     		    type:'post',  
     		    cache:false,   
