...
<ul class="nav navbar-default nav-pills nav-justified" style="font-size:16px;">
	...
	<li class="active"><a href="ListTeacherExperiments"><span style="color:white;">我的实验</span></a></li>
		...
		<div class="col-lg-12">
		  <section class="panel">
            ...
            <div class="tab-content">
                ...
                <input type="hidden" id="pageCount" value="${sessionScope.pagesize}"/>
                    <table class="table table-hover table-striped table-bordered table-responsive" id="table1">
				        <thead>
					       <tr>
				        ...
				        <tbody>
				        <c:forEach items="${sessionScope.listteacherexperiments}" var="listteacherexperiments">		
					       <tr id="tr1">
					       	<c:if test="${listteacherexperiments.cate=='现场现金实验'}">
						      <td style= "text-align:center;">
							     <a href="ListOneExperiments?id=${listteacherexperiments.id}" target="_blank" id="a1"><span id="span1">${listteacherexperiments.name}</span></a><br>
                                 	<i>（<span id="span5">${listteacherexperiments.pay}</span>  <span id="span6">¥</span>）</i>
						      </td>
						    </c:if>
						    <c:if test="${listteacherexperiments.cate=='现场学分实验'}">
						      <td style= "text-align:center;">
							     <a href="ListOneExperiments?id=${listteacherexperiments.id}" target="_blank" id="a1"><span id="span1">${listteacherexperiments.name}</span></a><br>
                               	  <i>（<span id="span5">${listteacherexperiments.credit}</span>  <span id="span6">学分</span>）</i>
						      </td>
						    </c:if>
						    <c:if test="${listteacherexperiments.cate=='在线实验'}">
						      <td style= "text-align:center;">
							     <a href="ListOneExperiments?id=${listteacherexperiments.id}" target="_blank" id="a1"><span id="span1">${listteacherexperiments.name}</span></a><br>
                                 	<i>（在线实验）</i>
						      </td>
                               ...
                               <td style= "text-align:center;">
                                 <span id="span2">${listteacherexperiments.starttime}</span>/<span id="span3">${listteacherexperiments.endtime}</span>
						      </td>
						      <td style= "text-align:center;">
							     <span class="glyphicon glyphicon-ok"><span id="span4">${listteacherexperiments.state}</span></span><br/>
							     <span class="glyphicon glyphicon-ok"><span id="span4">${listteacherexperiments.pubexper}</span></span>
						      </td>
						      <td style= "text-align:center;">
							     <a href="ListOneExperiments?id=${listteacherexperiments.id}" target="_blank" id="a2">实验信息</a><br>
						      </td>
						      <td style= "text-align:center;">
							     <a href="ListOneExperiments1?id=${listteacherexperiments.id}" target="_blank" id="a3">编辑</a><br>
							     <c:if test="${listteacherexperiments.state=='已完成'}">
							     		完成
							     </c:if>
							     <c:if test="${listteacherexperiments.state=='已创建'}">
							     <a target="_blank"  data-toggle="popover-x" href="ListOneExperiments3?id=${listteacherexperiments.id}" data-placement="top">完成</a>
							     </c:if>
						      </td>
					       </tr>
					    </c:forEach>
			         ...
			         <br>
			          <div class="tcdPageCode" style="float: right">
			          </div>
                </div>
        	...
        	$.ajax({
     		   ...
     		   data:{  current:p 
     		    ...
     		    success:function(data) { 
     		    	$("#table1 tbody").remove();
     		    	var jsonObj=eval(data.listteacherexperiments);
     		    	if(jsonObj[0]==null){
     		    		...
     		    		$.each(jsonObj, function (index, item) { 
     		    			 var $tr = $('<tr>');
     				    		...
     				    		var cate=item.cate;
     				    		if(cate=="现场学分实验"){
     				    			$tr.append("<td style= 'text-align:center;'><a target='_blank' href='ListOneExperiments?id="+item.id+"'><span >"+item.name+"</span></a><br><i>（<span>"+item.credit+"</span>  <span>学分</span>）</i></td>");
     				    		}
     				    		else if(cate=="现场现金实验"){
     				    			$tr.append("<td style= 'text-align:center;'><a target='_blank' href='ListOneExperiments?id="+item.id+"'><span >"+item.name+"</span></a><br><i>（<span>"+item.pay+"</span>  <span>¥</span>）</i></td>");
     				    		}
     				    		else{
     				    			$tr.append("<td style= 'text-align:center;'><a target='_blank' href='ListOneExperiments?id="+item.id+"'><span >"+item.name+"</span></a><br><i>（<span>在线实验</span>）</i></td>");
     				    		}
     				    		
     				    		$tr.append("<td style= 'text-align:center;'><span >"+starttime+"</span>/<span>"+endtime+"</span></td>");
     				    		$tr.append("<td style= 'text-align:center;'><span class='glyphicon glyphicon-ok'><span>"+item.state+"</span></span><br/>"+
     								     "<span class='glyphicon glyphicon-ok'><span id='span4'>"+item.pubexper+"</span></span></td>");
     				    		$tr.append("<td style= 'text-align:center;'><a target='_blank' href='ListOneExperiments?id="+item.id+"' >实验信息</a><br></td>");
     				    		if(item.state=="已完成"){
     				    			$tr.append("<td style= 'text-align:center;'><a target='_blank' href='ListOneExperiments1?id="+item.id+"' id='a3'>编辑</a><br>"+
     								     "<a target='_blank'   data-toggle='popover-x'  disabled>完成</a>")
     								     }
     				    		else{
     				    			$tr.append("<td style= 'text-align:center;'><a target='_blank' href='ListOneExperiments1?id="+item.id+"' id='a3'>编辑</a><br>"+
     				    					"<a target='_blank'  data-toggle='popover-x'  href='ListOneExperiments3?id="+item.id+"' data-placement='top'>完成</a>"+"</td>")
     				    		}
     		    			 	$tab1.append($tr);
