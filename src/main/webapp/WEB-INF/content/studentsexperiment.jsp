...
<ul class="nav navbar-default nav-pills nav-justified" style="font-size:16px;">
	...
	<li class="active"><a href="ListJOinExperiments"><span style="color:white;">我的实验</span></a></li>
	<li><a href="ListactiveExperiments3">有效实验</a></li>
	<li><a href="ListStudentsInfo" target="_blank">个人信息</a></li>
	<li><a href="login">退出登录&nbsp;<span class="glyphicon glyphicon-log-out"></span></a></li>
</ul>
...
<div class="row">
		<div class="col-lg-12">
		 <section class="panel">
			...
			<li role="presentation" class="active" style="font-size: 14px"><a href="#all2" role="tab" data-toggle="tab" >正在进行</a></li>  
            ...
            <div class="tab-content">
                <input type="hidden" id="pageCount2" value="${sessionScope.pagesize5}"/>
               ...
               <input type="hidden" id="pageCount" value="${sessionScope.pagesize4}"/>
                 <div role="tabpanel" class="tab-pane active" id="all2">
			         <table class="table table-hover table-bordered table-striped"   id="table1">
				        ...
				        <tbody>
					        ...
					        <c:forEach items="${sessionScope.liststudentexperiments}" var="liststudentexperiments">
					        <tr >
					       <c:if test="${liststudentexperiments.cate=='现场现金实验'}">
						      <td style= "text-align:center;">
							     <a target="_blank" href="ListOneExperiments2?id=${liststudentexperiments.id}" id="a1"><span id="span1">${liststudentexperiments.name}</span></a><br>
                                 <i>（<span id="span5">${liststudentexperiments.pay}</span>  <span id="span6">¥</span>）</i>
						      </td>
						    </c:if>
						    <c:if test="${liststudentexperiments.cate=='现场学分实验'}">
						      <td style= "text-align:center;">
							     <a target="_blank" href="ListOneExperiments2?id=${liststudentexperiments.id}" id="a1"><span id="span1">${liststudentexperiments.name}</span></a><br>
                                 <i>（<span id="span5">${liststudentexperiments.credit}</span>  <span id="span6">学分</span>）</i>
						      </td>
						    </c:if>
						    <c:if test="${liststudentexperiments.cate=='在线实验'}">
						      <td style= "text-align:center;">
							     <a target="_blank" href="ListOneExperiments2?id=${liststudentexperiments.id}" id="a1"><span id="span1">${liststudentexperiments.name}</span></a><br>
                                 <i>（在线实验）</i>
						      </td>
                              ...
                              <td style= "text-align:center;">
							     ${liststudentexperiments.starttime}/${liststudentexperiments.endtime}
						      </td>
						     
						       
						      <td style= "text-align:center;">
							     <span class="glyphicon glyphicon-ok">${liststudentexperiments.state}</span>
						      </td >
						      
						       
						      <td style= "text-align:center;">
							     <a target="_blank" href="ListOneExperiments2?id=${liststudentexperiments.id}">实验信息</a><br>
						      </td>
						    
						      <td style= "text-align:center;">
							    <span id="span1">实验进行中</span>
						      </td>
                 ...
                 <input type="hidden" id="pageCount3" value="${sessionScope.pagesize6}"/>
...
function activeexperiment(){
	...
	$.ajax({
		   url:'ListJOinedExperiments',// 跳转到 action  
		   data:{   
		    }, 
		    type:'post',  
		    cache:false,   
		    ...
		    success:function(data) { 
		    	$("#table2 tbody").remove();
		    	var jsonObj=eval(data.listjoinedexperiments);
		    	if(jsonObj==null){
		    		...
		    		$.each(jsonObj, function (index, item) { 
		    			 var $tr = $('<tr>');
				    		...
				    		var cate=item.cate;
				    		if(cate=="现场学分实验"){
				    			$tr.append("<td style= 'text-align:center;'><a target='_blank' href='ListOneExperiments2?id="+item.id+"'><span >"+item.name+"</span></a><br><i> （<span>"+item.credit+"</span>  <span>学分</span>）</i></td>");
				    		}
				    		else if(cate=="现场现金实验"){
				    			$tr.append("<td style= 'text-align:center;'><a target='_blank' href='ListOneExperiments2?id="+item.id+"'><span >"+item.name+"</span></a><br><i> （<span>"+item.pay+"</span>  <span>¥</span>）</></td>");
				    		}
				    		else{
				    			$tr.append("<td style= 'text-align:center;'><a target='_blank' href='ListOneExperiments2?id="+item.id+"'><span >"+item.name+"</span></a><br><i> （<span>在线实验</span>）</></td>");
				    		}
				    		...
				    		$tr.append("<td style= 'text-align:center;'><a target='_blank' href='ListOneExperiments2?id="+item.id+"' >实验信息</a><br></td>");
				    		$tr.append("<td style= 'text-align:center;'><span class='glyphicon glyphicon-ok'><span>已完成实验</span></span></td>");
		    			 	$tab1.append($tr);
...
function unactiveexperiment(){
	$.ajax({
		   url:'ListunJOinExperiments',// 跳转到 action  
		   data:{   
		    }, 
		    type:'post',  
		    cache:false,   
		    ...
		    success:function(data) { 
		    	$("#table3 tbody").remove();
		    	var jsonObj=eval(data.listunjoinexperiments);
		    	if(jsonObj==null){
		    		...
		    		$.each(jsonObj, function (index, item) { 
		    			 var $tr = $('<tr>');
				    		...
				    		if(cate=="现场学分实验"){
				    			...
				    			$tr.append("<td style= 'text-align:center;'><a target='_blank' href='ListOneExperiments2?id="+item.id+"'><span >"+item.name+"</span></a><br><i> （<span>"+item.pay+"</span>  <span>¥</span>）</i></td>");
				    		}
				    		else{
				    			$tr.append("<td style= 'text-align:center;'><a target='_blank' href='ListOneExperiments2?id="+item.id+"'><span >"+item.name+"</span></a><br><i> （<span >在线实验</span>）</i></td>");
				    		}
				    		
				    		$tr.append("<td style= 'text-align:center;'><span >"+starttime+"</span>/<span>"+endtime+"</span></td>");
				    		$tr.append("<td style= 'text-align:center;'><span class='glyphicon glyphicon-ok'><span>"+item.state+"</span></span></td>");
				    		$tr.append("<td style= 'text-align:center;'><a target='_blank' href='ListOneExperiments2?id="+item.id+"' >实验信息</a><br></td>");
				    		$tr.append("<td style= 'text-align:center;'><span class='glyphicon glyphicon-ok'><span>未完成实验</span></span></td>");
		    			 	$tab1.append($tr);
...
<script>
    $(".tcdPageCode1").createPage({
        ...
        backFn:function(p){
        	$.ajax({
     		   url:'ListJOinExperiments1',// 跳转到 action  
     		   data:{  current:p 
     		    }, 
     		    type:'post',  
     		    cache:false,   
     		    ...
     		    success:function(data) { 
     		    	$("#table1 tbody").remove();
     		    	var jsonObj=eval(data.liststudentexperiments);
     		    	if(jsonObj[0]==null){
     		    		...
     		    		$.each(jsonObj, function (index, item) { 
     		    			 var $tr = $('<tr>');
     				    		...
     				    		var cate=item.cate;
     				    		if(cate=="现场学分实验"){
     				    			$tr.append("<td><a target='_blank' href='ListOneExperiments2?id="+item.id+"'><span >"+item.name+"</span></a><br>（<span>"+item.credit+"</span>  <span>学分</span>）</td>");
     				    		}
     				    		else if(cate=="现场现金实验"){
     				    			$tr.append("<td><a target='_blank' href='ListOneExperiments2?id="+item.id+"'><span >"+item.name+"</span></a><br>（<span>"+item.pay+"</span>  <span>¥</span>）</td>");
     				    		}
     				    		else{
     				    			$tr.append("<td><a target='_blank' href='ListOneExperiments2?id="+item.id+"'><span >"+item.name+"</span></a><br>（<span>在线实验</span>）</td>");
     				    		}
     				    		
     				    		$tr.append("<td><span >"+starttime+"</span>/<span>"+endtime+"</span></td>");
     				    		$tr.append("<td><span class='glyphicon glyphicon-ok'><span>"+item.state+"</span></span></td>");
     				    		$tr.append("<td><a target='_blank' href='ListOneExperiments2?id="+item.id+"' >实验信息</a><br></td>");
     				    		$tr.append("<td style= 'text-align:center;'><span class='glyphicon glyphicon-ok'><span>实验进行中</span></span></td>");
     		    			 	$tab1.append($tr);
    ...
    $(".tcdPageCode2").createPage({
        ...
        backFn:function(p){
        	$.ajax({
     		   url:'ListJOinedExperiments1',// 跳转到 action  
     		   data:{   current:p 
     		    }, 
     		    type:'post',  
     		    cache:false,   
    ...
    $(".tcdPageCode3").createPage({
        ...
        backFn:function(p){
        	$.ajax({
     		   url:'ListunJOinExperiments1',// 跳转到 action  
     		   data:{  current:p  
     		    }, 
     		    type:'post',  
     		    cache:false,   
