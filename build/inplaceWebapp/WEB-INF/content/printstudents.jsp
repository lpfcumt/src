				...
				<style type="text/css" media=print>    
					...
					<header class="panel-heading">
					  <div dispay="table-caption">
						...
						<c:forEach items="${sessionScope.listexperiments2}" var="listexperiments2">
						<p>实验名称：${listexperiments2.name}</p>
						<p>老师：${listexperiments2.teachers_name}</p>
						<p>负责人：${listexperiments2.principals}</p>
						<c:if test="${listexperiments2.cate=='在线实验'}">
						<p>类别：${listexperiments2.cate}</p>
						</c:if>
						<c:if test="${listexperiments2.cate=='现场现金实验'}">
						<p>类别：${listexperiments2.cate}</p>
						<p>支付：${listexperiments2.pay} ¥</p>
						</c:if>
						<c:if test="${listexperiments2.principals=='现场学分实验'}">
						<p>类别：${listexperiments2.cate}</p>
						<p>支付：${listexperiments2.credit} 学分</p>
						</c:if>
						</c:forEach>
						<c:forEach items="${sessionScope.listscreenings3}" var="listscreenings3">
						<p>实验场次：${listscreenings3.screenings}</p>
						<p>实验时间：${listscreenings3.starttime}——${listscreenings3.endtime}</p>
						</c:forEach>
					  </div>
					</header>  
					<div class="panel-body"> 
					  <table class="table table-hover col-sm-6 table-bordered table-striped">
					  	...
					  	<tbody>
					  		<c:forEach items="${sessionScope.liststudents2}" var="liststudents2">
					  		<tr>
					  			<td style="vertical-align:middle; text-align:center;">${liststudents2.name}</td>
					  			<td style="vertical-align:middle; text-align:center;">${liststudents2.students_id}</td>
					  			<td style="vertical-align:middle; text-align:center;">${liststudents2.university}</td>
					  			<td style="vertical-align:middle; text-align:center;">${liststudents2.grade}-${liststudents2.major}</td>
					  			<td style="vertical-align:middle; text-align:center;">${liststudents2.tel}</td>
